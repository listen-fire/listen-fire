...
// This product app keeps `/` only as a router: logged-in users land on
// /dashboard; logged-out visitors are sent to /login. The public marketing
// landing lives in the standalone apps/marketing app, on its own host, and is
// reached directly rather than via this redirect.
//
// A single-tenant install may not run the unit /dashboard belongs to, so it lands on
...
export default function RootPage() {
  ...
  useEffect(() => {
    ...
    window.location.replace(
      capabilities?.identity === "static" ? firstMountedHref(capabilities) : "/dashboard",
    );
