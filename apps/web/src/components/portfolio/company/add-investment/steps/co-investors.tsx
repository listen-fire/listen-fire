...
export function CoInvestors() {
  ...
  useEffect(() => {
    setStepValidity("CO_INVESTORS", true);
  ...
  return (
    <div className="flex w-full flex-col items-stretch gap-4">
      ...
      <div className="flex flex-col items-stretch gap-4 rounded-lg border border-gray-100 bg-gray-50 p-3">
            ...
            <FormSelect<{ id: string; type: CoInvestorType }>
              ...
              load={async (inputValue) => {
                const investors = await getOtherInvestors({
                  entityId:
                    values.entity === "NEW" ? undefined : values.entity,
                  search: inputValue,
                });
      ...
      {values.coInvestors && values.coInvestors.length > 0 ? (
        <div className="flex flex-col items-stretch">
          {values.coInvestors.map((coInvestor, index) => (
            <div
              key={index}
              className="group flex items-center gap-4 border-b border-gray-100 py-2 last:border-b-0"
            >
              ...
              <button
                ...
                className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-red-500 opacity-0 transition-opacity hover:bg-red-50 focus:opacity-100 group-hover:opacity-100"
              >
        ...
        <div className="my-12 text-center text-[12px] text-gray-500">
          Add co-investors
        </div>
      )}
    </div>
