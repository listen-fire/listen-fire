...
import { runSystemAgent } from '../../../../lib/knowledge/system_agent';
...
const AGENT_TYPE = 'knowledge_system';

const systemAgentRouter = (procedure: typeof trpc.procedure) => {
  ...
  return trpc.router({
    ask: userProcedure
      ...
      .mutation(async ({ input: { message, sessionId, conversationId } }) => {
        ...
        backgroundWork().catch((err) => console.error('System agent background error:', err));
