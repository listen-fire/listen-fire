/* Sourced from https://gist.github.com/drakodev/e85c1fd6d9ac8634786d6139e0066fa0 */
...
const emailProviderLookup = new Map<string, boolean>(
  emailProviders.map((provider) => [provider, true]),
);

function emailsToCompanyUrls(emails: string[]) {
  const companyUrls = [];
  for (const email of emails) {
    const domain = email.split('@')[1];
    if (domain && !emailProviderLookup.has(domain)) {
      companyUrls.push(`https://${domain}`);
    }
  }

  return companyUrls;
}
export { emailsToCompanyUrls };
