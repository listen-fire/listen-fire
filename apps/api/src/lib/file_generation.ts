...
async function generateImage(args: {
  ...
  // On the Google route there IS no other vendor: no OpenAI account exists, so
  // reaching for one would build a client around a key that is deliberately
  ...
  if (openAiRoute() === 'google') {
    throw new Error(
      `The OpenAI route is google, so images are generated by Gemini — but the Google service account ` +
        `is not configured (set ${missingGoogleServiceAccountVars().join(', ')}).`,
    );
  }
