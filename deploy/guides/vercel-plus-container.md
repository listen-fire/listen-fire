...
**Postgres, Redis, storage.** Managed Postgres 16 anywhere you like — and **run `deploy/postgres-init/00-roles.sql` against it by hand, as a superuser, before the first deploy**. The migration set grants to two roles (`agent`, `readonly`) that no managed provider creates for you, and a database without them dies on an early migration having built almost nothing. Redis is configured with `MESSAGE_QUEUE_REDIS_HOSTNAME` and `MESSAGE_QUEUE_REDIS_PORT` — there is no URL form, and the client takes a host and a port and nothing else, so a managed Redis requiring AUTH or TLS cannot be configured through them. Object storage is any S3-compatible bucket; there is no local-filesystem driver, and without one configured the process boots normally and only file-touching steps fail, naming the variables they wanted.
