...
One mailbox, not a sign-in. Nobody authorises anything and there is no consent screen, so Google's verification review does not apply: enable the Gmail API in the same Google Cloud project as the runtime service account, then have a Workspace admin add that account's client id under Security, then API controls, then Domain wide delegation, with the read scope `https://www.googleapis.com/auth/gmail.readonly` and, to send or reply, also `https://www.googleapis.com/auth/gmail.send`. The send scope is optional — a mailbox granted only the read scope still runs on arrival and can be searched, it just cannot send or reply until the admin adds the second scope.

Domain wide delegation has no per-mailbox limit of its own: once granted, the service account can act as ANY mailbox in the Workspace, not only the one you meant to connect. `GMAIL_MAILBOX_ALLOWLIST` is this deployment's own limit on top of that grant — set it to the comma separated addresses automations may act as. It is REQUIRED: unset or empty, the connector refuses every mailbox rather than defaulting to "every mailbox is fine."

Minutes, once the admin is in the room. After that a team connects Gmail by typing the mailbox address, which must be a real user or shared mailbox (a group address has no inbox to read) and must also be on `GMAIL_MAILBOX_ALLOWLIST`; the form calls Google as that mailbox before saving, so a delegation that was never granted, or an address this deployment hasn't allowlisted, is corrected on the form rather than in a run a week later.

The two scopes are the whole of what the mailbox can be granted, and each is checked independently — a mailbox can read without being able to send, or the reverse. Automations can search it, run when mail arrives, and send or reply as it — and can never label, archive, delete or draft. Mail is POLLED about once a minute, so there is no Pub/Sub topic, no push endpoint and no weekly watch to renew.
...
The per-system credentials for Slack, Attio, Airtable, Google, Dropbox, Telegram, WhatsApp and Twilio are listed with what they do in "Registering your own third-party apps". Every one is optional, and an unset OAuth pair means that system is simply not offered — except Attio's, where it means Attio connects with a pasted access token instead. Gmail takes no OAuth pair of its own: it rides the same service account as the rest of the deployment's Google access. It does take one variable, `GMAIL_MAILBOX_ALLOWLIST`, and it is required — see "Gmail" above.
