...
#
# Which key is the missing one follows MODEL_ROUTE: on `google` there is no
# vendor key at all — every call goes through the deployment's one Google
# service account — so a vendor key is never asked for there.
if [ "${LISTEN_FIRE_DEMO:-0}" != "1" ]; then
  if [ "${MODEL_ROUTE:-direct}" = "google" ]; then
    if [ -z "${GOOGLE_PRIVATE_KEY:-}" ] ||
       [ -z "${GOOGLE_CLIENT_EMAIL:-}" ] ||
       [ -z "${GOOGLE_PROJECT_ID:-}" ]; then
      echo "[init] WARNING: MODEL_ROUTE=google but the Google service account is not configured." >&2
